...
public class GridAdapter extends BaseAdapter {
    ...
    public GridAdapter(Activity activity, Gallery data) {
    ...
    @Override
    public int getCount() {
        return data.getSubInfos().size();
    }

    @Override
    public Object getItem(int position) {
        return data.getSubInfos().get(position);
    }
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        ...
        if(data.getSubInfos().get(position).getAlbum_name()!=null) {
            holder.imgnameTV.setText(data.getSubInfos().get(position).getAlbum_name().toString());
        }
        ...
        holder.grid_item_image.setErrorImageResId(R.drawable.ic_error);
          ...
          holder.grid_item_image.setImageUrl(Constants.ImageBaseUrl+data.getSubInfos().get(position).getTiny_image().toString().replace(" ","%20"),imageLoader);
        return view;
    }
